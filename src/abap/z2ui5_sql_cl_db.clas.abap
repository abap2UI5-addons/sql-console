CLASS z2ui5_sql_cl_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    "! Vendored copy of the abap2UI5 persistence helpers this app uses, so
    "! the app carries its own DB access instead of depending on
    "! z2ui5_cl_util_db. Reads and writes the shared z2ui5_t_91 table.
...
CLASS z2ui5_sql_cl_db IMPLEMENTATION.
  ...
  METHOD load_by_handle.
    ...
    z2ui5_sql_cl_context=>xml_parse(
      EXPORTING
        xml = lv_data
      IMPORTING
        any = result ).
  ...
  METHOD save.
    ...
    TRY.
        lv_data = z2ui5_sql_cl_context=>xml_stringify( data ).
    ...
    IF lv_id IS NOT INITIAL.
      ls_db-id = lv_id.
    ELSE.
      ls_db-id = z2ui5_sql_cl_context=>uuid_get_c32( ).
    ENDIF.
